                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="wupinguashiName" id="wupinguashiName" placeholder="物品名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="shiwuzhaolingTypes" id="shiwuzhaolingTypes" lay-filter="shiwuzhaolingTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in shiwuzhaolingTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="row">
                            <div class="col-md-3" v-for="(item,index) in dataList" >
                                <div class="galry-grid grid-top-rgt">
                                    <a href="javascript:return false;"  @click="jumpCheck('../wupinguashi/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                                        <img class="img-responsive zoom-img" :src="item.wupinguashiPhoto?item.wupinguashiPhoto.split(',')[0]:''" style="height: 350px;width:100%;"/>
                                        <div class="w3agile-text">
                                            <h5>
                                                <span>{{item.wupinguashiName}}</span>
                                                <br/>
                                                <span v-if="item.wupinguashiNewMoney">
...
<script>
    ...
    // 当前表的字典表
        ...
        function shiwuzhaolingTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=shiwuzhaoling_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function statusTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=status_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#wupinguashiName').val() != null && jquery('#wupinguashiName').val() != "" && jquery('#wupinguashiName').val() != "null" ) {
                param['wupinguashiName'] = jquery('#wupinguashiName').val();
            }
         
            var shiwuzhaolingTypes = $("#shiwuzhaolingTypes").find("option:selected").val();
            if (shiwuzhaolingTypes != null && shiwuzhaolingTypes != "" && shiwuzhaolingTypes != "null") {
                param['shiwuzhaolingTypes'] = shiwuzhaolingTypes;
            }
         
            var statusTypes = $("#statusTypes").find("option:selected").val();
            if (statusTypes != null && statusTypes != "" && statusTypes != "null") {
                param['statusTypes'] = statusTypes;
            }
                                                

            // 获取列表数据
            http.request('wupinguashi/list', 'get', param, function(res) {
