            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑失物招领留言</li>
                    </ol>
                </div>
            </div>
        </div>
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                    <div class="form-group col-md-6 shiwuzhaolingLiuyanTextDiv">
                                        <label>留言内容</label>
                                        <textarea style="width: 450px"  id="shiwuzhaolingLiuyanText" name="shiwuzhaolingLiuyanText" class="form-control"placeholder="留言内容"></textarea>
                                    </div>
...
<script>
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("shiwuzhaolingLiuyan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                yonghuId: "required",
                shiwuzhaolingLiuyanText: "required",
                replyText: "required",
            },
            messages: {
                shiwuzhaolingId: "物品不能为空",
                yonghuId: "用户不能为空",
                shiwuzhaolingLiuyanText: "留言内容不能为空",
                replyText: "回复内容不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addshiwuzhaolingLiuyan = window.sessionStorage.getItem("addshiwuzhaolingLiuyan");
        if (addshiwuzhaolingLiuyan != null && addshiwuzhaolingLiuyan != "" && addshiwuzhaolingLiuyan != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("shiwuzhaolingLiuyan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#shiwuzhaolingLiuyanText").val(ruleForm.shiwuzhaolingLiuyanText);
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            $('#shiwuzhaolingLiuyanText').attr('readonly', 'readonly');
